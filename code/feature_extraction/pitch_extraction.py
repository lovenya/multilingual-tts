...
def nemo_extract_pitch(file_path, sr=16000, frame_period=5.0):
    """
...
def process_language_folder_pitch(lang_folder: Path, sr=16000, frame_period=5.0):
    ...
    # Create target directory for pitches.
    ...
    for wav_file in wav_files:
        ...
        # Skip if pitch is already extracted.
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract pitch from all language folders in the dataset."
    )
    ...
    for lang_folder in language_folders:
        process_language_folder_pitch(lang_folder, sr=sr, frame_period=frame_period)
